...
export class TokenService implements ITokenService {
  ...
  private _inviteSecretKey: Secret;
  private _inviteExpiresIn: string;
  ...
  constructor() {
    ...
    this._inviteSecretKey = config.jwt.INVITE_SECRET_KEY;
    this._inviteExpiresIn = config.jwt.INVITE_EXPIRES_IN;
  }
  ...
  generateInviteToken(payload: {
    ...
    if (!this._inviteSecretKey || this._inviteSecretKey === "") {
      throw new CustomError(
        HTTP_STATUS.INTERNAL_SERVER_ERROR,
        "Invite token secret key is not configured. Please set INVITE_SECRET_KEY environment variable."
      );
    }
    return jwt.sign(payload, this._inviteSecretKey, {
      expiresIn: this._inviteExpiresIn as ms.StringValue,
    });
  }
